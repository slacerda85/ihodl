...
import { ElectrumMethod, ElectrumResponse, GetHistoryResult } from '@/shared/models/electrum'
import { JsonRpcRequest } from '@/shared/models/rpc'
import { randomUUID } from 'expo-crypto'
import { Tx } from '@/shared/models/transaction'
import { sha256 } from '../crypto'
import { bitcoin } from '../networks'
import { toOutputScript } from '../address/address'
...
export default class ElectrumService {
  ...
  static addressToScriptHash(address: string): string {
    ...
    // Create a reversed copy of the hash as a Uint8Array
    const hashArray = new Uint8Array(hash)
    const reversedHash = new Uint8Array(hashArray.length)
    for (let i = 0; i < hashArray.length; i++) {
      reversedHash[i] = hashArray[hashArray.length - 1 - i]
    }

    return uint8ArrayToHexString(reversedHash)
  }
   ...
   */
  static async getBalance(address: string, socket?: TLSSocket): Promise<ElectrumResponse<number>> {
    try {
      // Create a socket if not provided to reuse for multiple calls
      const managedSocket = !socket
      ...
      try {
        ...
        const balance = (balanceData.result.confirmed + balanceData.result.unconfirmed) / 100000000
